...
export function buildContextValues(ctx: TemplatingContext): Record<string, string> {
  ...
  const v: Record<string, string> = {
    ...
    contractor_rate: sr.agreed_hourly_rate != null ? `$${Number(sr.agreed_hourly_rate).toLocaleString()}` : "",
    contractor_hours: sr.agreed_hours != null ? `${sr.agreed_hours}` : "",
    contractor_total: sr.agreed_total != null ? `$${Number(sr.agreed_total).toLocaleString()}` : "",
