...
BEGIN
  ...
  -- 1) Staff match: any non-client role on a user whose email = v_email
  ...
  IF v_staff_role IS NOT NULL THEN
    UPDATE public.profiles SET role = v_staff_role, email = v_email WHERE id = v_uid;
    INSERT INTO public.user_roles(user_id, role) VALUES (v_uid, v_staff_role)
      ON CONFLICT (user_id, role) DO NOTHING;
    -- drop the auto-assigned 'client' role from the trigger
    DELETE FROM public.user_roles WHERE user_id = v_uid AND role = 'client';
  ...
  IF v_client_id IS NOT NULL THEN
    UPDATE public.profiles SET role = 'client', email = v_email WHERE id = v_uid;
  ...
  -- 3) DENY: revoke speculative profile/roles and remove the auth row so the
  -- unknown Google identity cannot retry against a half-linked state.
