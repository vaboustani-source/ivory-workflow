...
BEGIN
  ...
  -- 1) Staff match by email on an existing profile with a staff role
  ...
  IF v_staff_role IS NOT NULL THEN
    -- Bypass the profile role-change trigger by removing + re-inserting the row.
  ...
  -- 2) Couple match: clients table, existing client_users, or pending portal_invitation
  ...
  IF v_client_id IS NOT NULL THEN
    -- Profile already exists as 'client' from handle_new_user, leave role alone.
  ...
  -- 3) DENY: revoke speculative rows and the auth identity
  DELETE FROM public.user_roles WHERE user_id = v_uid;
  DELETE FROM public.profiles WHERE id = v_uid;
  BEGIN
